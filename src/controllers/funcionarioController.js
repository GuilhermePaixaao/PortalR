...
export const login = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Login realizado com sucesso!", data: dadosUsuario });
  } catch (error) {
...
export const criarUsuario = async (req, res) => {
  try {
    ...
    const dadosParaCriar = {
      ...
      senhaHash: senhaHash,
      cargoId: parseInt(funcionario.cargo.idCargo)
    };

    const novoId = await FuncionarioModel.create(dadosParaCriar);
    const novoFuncionario = await FuncionarioModel.findById(novoId); // Busca o usuário recém-criado
...
export const listarUsuarios = async (req, res) => {
  try {
    ...
    const usersFormatado = users.map(user => ({
      ...
      cargo: { idCargo: user.cargoId, nome: user.nomeCargo }
    }));
...
export const atualizarUsuario = async (req, res) => {
  try {
    ...
    if (funcionario.senha && funcionario.senha.trim() !== "") {
      const salt = await bcrypt.genSalt(10);
      const senhaHash = await bcrypt.hash(funcionario.senha, salt);
      sql = `UPDATE Funcionario SET nomeFuncionario = ?, usuario = ?, email = ?, senha = ?, cargoId = ? WHERE id = ?`;
      values = [funcionario.nomeFuncionario, funcionario.usuario, funcionario.email, senhaHash, parseInt(funcionario.cargo.idCargo)];
    } else {
      sql = `UPDATE Funcionario SET nomeFuncionario = ?, usuario = ?, email = ?, cargoId = ? WHERE id = ?`;
      values = [funcionario.nomeFuncionario, funcionario.usuario, funcionario.email, parseInt(funcionario.cargo.idCargo)];
    }

    const result = await FuncionarioModel.update(idNum, sql, values);
    ...
    res.status(200).json({ success: true, data: dadosUsuarioSemSenha });
